 ...
 */
int
block_part_read_block_part(struct cloudbacker_store *cb, cb_block_t block_num,
    u_int block_size, u_int off, u_int len, void *dest)
{
    ...
    if (off == 0 && len == block_size)
        return (*cb->read_block)(cb, block_num, dest, NULL, NULL, 0);
    ...
    /* Read entire block */
    if ((r = (*cb->read_block)(cb, block_num, buf, NULL, NULL, 0)) != 0) {
        free(buf);
        return r;
    }
 ...
 */
int
block_part_write_block_part(struct cloudbacker_store *cb, cb_block_t block_num,
    u_int block_size, u_int off, u_int len, const void *src)
{
    ...
    if (len == 0)
        return 0;
    if (off == 0 && len == block_size)
        return (*cb->write_block)(cb, block_num, src, NULL, NULL, NULL);
    ...
    if ((r = (*cb->write_block)(cb, block_num, buf, NULL, NULL, NULL)) != 0) {
        free(buf);
        return r;
    }
