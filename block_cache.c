...
struct block_cache_private {
    ...
    u_int                           write_block_err_count; // number of blocks failed to write successfully
...
block_cache_create(struct block_cache_conf *config, struct cloudbacker_store *inner)
{
    ...
    cb->list_blocks = block_cache_list_blocks;
    cb->flush = block_cache_flush;
    cb->destroy = block_cache_destroy;
    ...
    priv->config = config;
    priv->inner = inner;
    priv->err_write_block = 0; // will be set with error code, when atleast one block write fails
...
block_cache_dcache_load(void *arg, cb_block_t dslot, cb_block_t block_num, const u_char *md5)
{
    ...
    if ((entry = cb_hash_get(priv->hashtable, block_num)) != NULL) {
        (*config->log)(LOG_ERR, "corrupted cache file: block 0x%0*jx listed twice (in dslots %ju and %ju)",
          CB_BLOCK_NUM_DIGITS, (uintmax_t)block_num, (uintmax_t)entry->u.dslot, (uintmax_t)dslot);
        return EINVAL;
    }
...
static int
block_cache_set_mounted(struct cloudbacker_store *cb, int *old_valuep, int new_value)
{
    struct block_cache_private *const priv = cb->data;
...
block_cache_flush(struct cloudbacker_store *const cb, int stop)
{
    ...
    if(priv->err_write_block != 0) {
        r = priv->err_write_block;    // return error
        (*config->log)(LOG_ERR, "flusing blocks - incomplete");
        (*config->log)(LOG_DEBUG, "number of blocks remaining to be flushed : %u", priv->write_block_err_count); 
    }
    else {
        r = 0;
        (*config->log)(LOG_INFO, "flushing blocks - completed");
    }
...
static void
block_cache_destroy(struct cloudbacker_store *const cb)
{
    struct block_cache_private *const priv = cb->data;
    struct block_cache_conf *const config = priv->config;
...
void
block_cache_get_stats(struct cloudbacker_store *cb, struct block_cache_stats *stats)
{
    struct block_cache_private *const priv = cb->data;
...
static int
block_cache_list_blocks(struct cloudbacker_store *cb, block_list_func_t *callback, void *arg)
{
    struct block_cache_private *const priv = cb->data;
...
static int
block_cache_read_block(struct cloudbacker_store *const cb, cb_block_t block_num, void *dest,
  u_char *actual_md5, const u_char *expect_md5, int strict)
{
    struct block_cache_private *const priv = cb->data;
    struct block_cache_conf *const config = priv->config;
...
static int
block_cache_read_block_part(struct cloudbacker_store *cb, cb_block_t block_num, u_int off, u_int len, void *dest)
{
    struct block_cache_private *const priv = cb->data;
...
static int
block_cache_write_block(struct cloudbacker_store *const cb, cb_block_t block_num, const void *src, u_char *md5,
  check_cancel_t *check_cancel, void *check_cancel_arg)
{
    struct block_cache_private *const priv = cb->data;
    struct block_cache_conf *const config = priv->config;
...
static int
block_cache_write_block_part(struct cloudbacker_store *cb, cb_block_t block_num, u_int off, u_int len, const void *src)
{
    struct block_cache_private *const priv = cb->data;
        ...
        if ((entry = TAILQ_FIRST(&priv->dirties)) != NULL && (priv->stopping || priv->flush || adjusted_now >= entry->timeout)) {
            ...
            if(r != 0) {
                ...
                if(r == ENXIO || r == EACCES || r == EIO ||  r == ECONNABORTED) {                    
                    ...
                    (*config->log)(LOG_ERR,"error writing block %0*jx : %s, will retry", CB_BLOCK_NUM_DIGITS, (uintmax_t)entry->block_num, strerror(r));
                    continue;
                }
               ...
               (*config->log)(LOG_ERR,"error writing block %0*jx : %s", CB_BLOCK_NUM_DIGITS, (uintmax_t)entry->block_num, strerror(r));
                priv->write_block_err_count++;  /* update for each write block failure with non retriable errors */
