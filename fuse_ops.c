...
fuse_op_init(struct fuse_conn_info *conn)
{
    ...
    if ((priv->cb = cloudbacker_create_store(cbconf)) == NULL) {
    ...
    /* Done */
    (*config->log)(LOG_INFO, "mounting %s", cbconf->mount);
    return priv;
}
...
fuse_op_destroy(void *data)
{
    ...
    if (priv == NULL || cb == NULL)
        return;
    (*config->log)(LOG_INFO, "unmount %s: initiated", cbconf->mount);
    ...
    /* Clear mounted flag */
    if (!config->read_only) {
        (*config->log)(LOG_INFO, "unmount %s: clearing mounted flag", cbconf->mount);
        if ((r = (*cb->set_mounted)(cb, NULL, 0)) != 0)
            (*config->log)(LOG_ERR, "unmount %s: clearing mounted flag failed: %s", cbconf->mount, strerror(r));
    }

    /* Shutdown */
    (*cb->destroy)(cb);
    (*config->log)(LOG_INFO, "unmount %s: completed", cbconf->mount);
    free(priv);
}
    ...
    struct fuse_file_info *fi)
{
    ...
    if ((offset & mask) != 0) {
        ...
        if ((r = (*priv->cb->read_block_part)(priv->cb, block_num, fragoff, fraglen, buf)) != 0)
            return -r;
        buf += fraglen;
        offset += fraglen;
        size -= fraglen;
    ...
    while (num_blocks-- > 0) {
        if ((r = (*priv->cb->read_block)(priv->cb, block_num++, buf, NULL, NULL, 0)) != 0)
            return -r;
    ...
    if ((size & mask) != 0) {
        ...
        if ((r = (*priv->cb->read_block_part)(priv->cb, block_num, 0, fraglen, buf)) != 0)
            return -r;
    }
    ...
    off_t offset, struct fuse_file_info *fi)
{
    ...
    if ((offset & mask) != 0) {
        ...
        if (fraglen > size)
            fraglen = size;
        block_num = offset >> priv->block_bits;
        if ((r = (*priv->cb->write_block_part)(priv->cb, block_num, fragoff, fraglen, buf)) != 0)
            return -r;
        buf += fraglen;
        offset += fraglen;
        size -= fraglen;
    ...
    while (num_blocks-- > 0) {
        if ((r = (*priv->cb->write_block)(priv->cb, block_num++, buf, NULL, NULL, NULL)) != 0)
            return -r;
        buf += config->block_size;
    }
    ...
    if ((size & mask) != 0) {
        ...
        if ((r = (*priv->cb->write_block_part)(priv->cb, block_num, 0, fraglen, buf)) != 0)
            return -r;
    }
...
*/
    ...
    if ((offset & mask) != 0) {
        ...
        if (fraglen > size)
            fraglen = size;
        block_num = offset >> priv->block_bits;
        if ((r = (*priv->cb->write_block_part)(priv->cb, block_num, fragoff, fraglen, zero_block)) != 0) {
            free(zero_block);
            return -r;
        }
        offset += fraglen;
    ...
    /* Write intermediate complete blocks */
    while (num_blocks-- > 0) {
        if ((r = (*priv->cb->write_block)(priv->cb, block_num++, NULL, NULL, NULL, NULL)) != 0) {
            free(zero_block);
            return -r;
        }
    }
    ...
    if ((size & mask) != 0) {
        ...
        if ((r = (*priv->cb->write_block_part)(priv->cb, block_num, 0, fraglen, zero_block)) != 0) {
            free(zero_block);
            return -r;
        }
    }
