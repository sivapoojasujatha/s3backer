...
int
cloudbacker_erase(cloudbacker_config *config)
{
    ...
    if ((priv->backerstore = config->test ? test_io_create(&config->http_io) : http_io_create(&config->http_io)) == NULL) {
        warnx(config->test ? "test_io_create" : "http_io_create");
        goto fail3;
    }

    /* Iterate over non-zero blocks */
    if ((r = (*priv->backerstore->list_blocks)(priv->backerstore, erase_list_callback, priv)) != 0) {
        warnx("can't list blocks: %s", strerror(r));
        goto fail3;
    }

    /* Clear mounted flag */
    if ((r = (*priv->backerstore->set_mounted)(priv->backerstore, NULL, 0)) != 0) {
...
fail3:
    ...
    if (priv->backerstore != NULL) {
        if (ok && !config->quiet) {
            fprintf(stderr, "done\n");
            warnx("erased %ju non-zero blocks", priv->count);
        }
        (*priv->backerstore->destroy)(priv->backerstore);
    }
...
erase_thread_main(void *arg)
{
    ...
    while (1) {
        ...
        if (priv->qlen > 0) {
            ...
            /* Do block deletion */
            pthread_mutex_unlock(&priv->mutex);
            r = (*priv->backerstore->write_block)(priv->backerstore, block_num, NULL, NULL, NULL, NULL);
            pthread_mutex_lock(&priv->mutex);
